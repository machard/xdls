...
var xds = function(url, callback){
	serverUrl =  url; //"../xds/xds-serve/xds-serve.html";
...
var replaceLocalStorage = function () {
	...
	var checkReadyAndCall = function(fn) {
		return function() {
			if(!server) throw "You need to wait until ready callback has fired";
...
var post = function(request){
	if(!server || processingKeys.indexOf(request.key)>=0) return queue.push(request);
